...
from .models import University, AdmissionNews, Comment, Department, Discussion, Answer
from .forms import CommentForm, AdmissionNewsCreateForm, DiscussionCreateForm, AnswerForm
...
class AdmissionNewsDetailView(FormMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        Event.objects.get_or_create(title=title, description=description, start_time=start_time, end_time=end_time,
                                    admissionnews=post)
...
class DiscussionListView(ListView):
    model = Discussion
    template = 'admissionnews/discussion_list.html'
...
class DiscussionDetailView(FormMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['form'] = self.get_form()
        discussion = Discussion.objects.filter(pk=self.kwargs.get('pk'))[0]
        title = discussion.title
        description = discussion.details
        context['answers'] = Answer.objects.filter(discussion=discussion).order_by('-id')
        return context
    ...
    def form_valid(self, form):
        form.instance.user = self.request.user
        form.instance.discussion = Discussion.objects.filter(
            pk=self.kwargs.get('pk')).first()
        # comment, created = Comment.objects.get_or_create(**form.cleaned_data)
        return super().form_valid(form)
...
class DiscussionDeleteView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    ...
    def test_func(self):
        ...
        if self.request.user.is_superuser or self.request.user == obj.user:
            return True
        return False
...
class AnswerDeleteView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    ...
    def get_success_url(self, **kwargs):
        return reverse_lazy('discussion-detail', args=(self.object.post.id,))
